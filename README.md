...
5. 热点开启方式说明: 推荐用 api(默认)
     ...
     - api 的最后一层兜底是 UI 点击: 前两层都开不了时会亮屏、打开系统的热点设置页去点热点开关，所以会看到弹出一个热点界面。不想要可以把 config.json 里的 ap_ui_fallback 设为 false
...
- **HotspotPlus_v8.6**
      ...
      1. 修复热点明明已经开了，模块却认为没开(日志写"三层全部尝试后热点仍未开启")。原因是热点检测只认 ap0/wlan1 等几个网卡名，
         iQOO/vivo(高通) 的热点网卡叫 wlan2，不在名单里。现在先问系统的网络共享服务(dumpsys tethering)哪个 Wi-Fi 网卡在共享，不再依赖网卡名；
         还认不出来时再经 hotspotctl.dex 问 WifiManager 的热点状态，名单只做兜底
      2. 因为上面的误判，层1 其实已经开好的热点又被拿去跑层2 和层3: 层2 碰上已开的热点只会报 "Soft AP failed to start"，
         层3 则去亮屏打开设置点击。现在每进下一层前都会先确认热点是不是已经开了(或正在开启)，开了就停
      3. 修复层3(UI 点击)点错位置: 原来拆分界面 dump 的 `tr '>' '>\n'` 没有真正换行，整份 dump 被当成一行，
         取到的是整个屏幕的范围，于是每次都点在屏幕正中间(iQOO Z5 上正好点到 USB 网络共享那一行)。
         现在按行找到标题是"WLAN 热点/个人热点/Wi-Fi hotspot"那一行的开关再点；开关已经是开的就不再点(避免把热点点关)；
         vivo/OriginOS 这种第一页只有"WLAN 热点"入口的，会先点进去再找开关；不在系统设置界面、或者只找到 USB/蓝牙/以太网共享的开关时一律不点
      4. 开完热点会在 log/open_hotspot.log 里写明热点名称，以及用的是系统设置里的名称/密码还是 config.json 里 ap_mode2 的
      5. hotspotctl.dex 新增 state 动作(查询热点状态和系统保存的热点名称)；定时热点检测(check.sh ap)也改用新的检测，
         不会再因为认不出网卡去重复开热点、切飞行模式
      6. 模式一(mode1)的热点检测也从只认 ap0 改为通用检测
      7. 开机后自己弹出一个"热点和网络共享"界面(和手机设置里的热点界面长得不一样): 这是上面第 1 条误判后跑到了层3，
         层3 打开的是原生安卓隐藏的 TetherSettings 页面，vivo 等 ROM 平时不显示这一页。误判修掉后，热点能开的机型不会再走到层3。
         真走到层3 时也做了这些调整，尽量适配各家 ROM:
         - 先打开系统的"WLAN 热点设置"入口(安卓 11+)，各家 ROM 会指到自家的热点页，也就是下拉快捷开关里长按"热点"进去的那一页；
           打不开(安卓 10 及以下等)再用原生的 TetherSettings
         - 亮屏改用 WAKEUP 键(原来用电源键，屏幕本来亮着时会被按灭)；屏幕本来亮着且没锁屏时不再上滑(原来会在正在用的 App 里乱滑)；
           解锁先用 wm dismiss-keyguard，上滑坐标按屏幕分辨率算；原来灭屏的，弄完会灭回去
         - 热点开好(或者层3 放弃)后自动退出设置页，不再停留: 先按返回键把刚打开的页面一层层退掉(不留在最近任务里，
           屏幕本来亮着的话会回到之前正在用的 App)；返回键退不掉就按 HOME，HOME 键被 ROM 拦掉再用 HOME intent 回桌面。
           热点一开就退出，不等后面的日志；脚本被中途打断也会退出设置页。退出方式写在 log/open_hotspot.log 里
         - 锁屏设了密码/图案时模块解不开，这时不再打开设置页(原来会把设置页压在锁屏后面，锁屏上 HOME 键也不管用，
           用户解锁后反而看到这个页面)，收起弹出的密码界面，原来灭屏的灭回去
         - 新增 ap_ui_fallback(默认 true)，设为 false 就完全不做 UI 点击、不会弹出界面
      8. 热点检测以系统的回答为准，判断顺序: 系统网络共享服务 -> WifiManager -> 两个都答不上来才按网卡名单猜。
         高通机型开了"双 WLAN 加速"时 wlan1 是第二个 Wi-Fi 连接，原来会被误认成热点已开，导致热点其实没开也不去开
      9. 层2 的热点名称带空格时不再被拆开；日志里不再写明文热点密码
      10. hotspotctl.dex: 安卓 12+ 的开热点请求明确指定"带网络共享"(connectivityScope=GLOBAL)；兼容新系统把参数换成 TetheringRequest 的情况
